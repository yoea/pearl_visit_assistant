...
10. 数据合理性检查：对数值字段做常识性检查——height / weight（明显超出生理常识范围）、distanceToSchoolKm（异常，如超过 100 公里）、annualIncome / perCapitaIncome（与 schoolChildrenCount 等明显不匹配）、zhongkaoScore / zhongkaoFullScore（成绩大于满分、为 0 或负值）、schoolChildrenCount（数值异常）。单生数值异常 → 写入该生 informationToVerify（如「身高填写 1800cm，疑似单位或录入错误，需核实」）；多生普遍异常 → 汇总至 schoolAnalysis.dataQualityIssues。基于异常数值的推测只能写入 interviewNotes 且以「推测：」开头。null 字段按规则 6 处理，不得臆测补值。规则 8-10 的全部结论一律归入现有 JSON 字段，不得新增任何 JSON 字段。
