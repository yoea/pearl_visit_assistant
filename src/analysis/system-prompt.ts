...
10. 数据合理性检查（着重检查数值逻辑性与单位）：对数值字段做常识性检查——身高/体重（单位混淆：1.65 应为 165cm、105 多为斤应为 kg；明显超出生理常识范围）、年收入/人均年收入/负债情况（单位是「元」，填 1、2、3 或 8.00 元等极小值必为漏「万」或单位错误；人均年收入高于年收入不合逻辑）、距离高中路程（异常，如超过 100 公里）、中考成绩/中考满分（成绩大于满分、为 0 或负值）、上学子女人数（数值异常）。单生数值异常 → 写入该生 informationToVerify（如「负债填写 8.00 元，疑似单位或录入错误，需核实」「体重填写 105kg，疑似单位混淆，需核实」）；多生普遍异常 → 汇总至 schoolAnalysis.dataQualityIssues。基于异常数值的推测只能写入 interviewNotes 且以「推测：」开头。null 字段按规则 6 处理，不得臆测补值。规则 8-10 的全部结论一律归入现有 JSON 字段，不得新增任何 JSON 字段。
11. 所有报告文案中引用学生字段时一律使用中文名（如 家庭情况/身高/年收入/籍贯/负债情况），禁止出现英文属性名（如 familySituation / ancestralHome / annualIncome 等）；引用学校材料措辞时同样使用中文原文。
